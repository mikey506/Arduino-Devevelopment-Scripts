...
const char* ssid = "Mikey-So-Fly";
const char* password = "Mikester3";
...
void loop() {
  if (!clients[numClients] || !clients[numClients].connected()) {
    ...
    if (clients[numClients]) {
      ...
      clients[numClients].println("Welcome to the Pocket / BBS!");
...
void scanWifi(WiFiClient client) {
  ...
  // Iterate over sorted networks and print their details
